  ...
  def update
    ...
    if @employee.save
      if employee_params[:expected].present?
        if Schedule.find_by(employee_name: current_employee.name, employee_id: current_employee.id, leave_stage: 0).present?
          # expectedの入力が2回目以降の場合、updateで名前を表示させる日付を更新する
          Schedule.find_by(employee_name: current_employee.name, employee_id: current_employee.id, leave_stage: 0).update(start_time: @employee.expected - 41)
        else
          #expecteを初めて入力した場合に実行する
          Schedule.create(employee_name: current_employee.name, employee_id: current_employee.id, leave_stage: 0, start_time: @employee.expected - 41)
        end
      end
      ...
      if employee_params[:birth].present?
        ...
        stage_days.each_with_index do |day, i|
          if Schedule.find_by(employee_name: current_employee.name, employee_id: current_employee.id, leave_stage: i+1).present?
            Schedule.find_by(employee_name: current_employee.name, employee_id: current_employee.id, leave_stage: i+1).update(start_time: @employee.birth + day)
          else
            Schedule.create(employee_name: current_employee.name,  employee_id: current_employee.id, leave_stage: i+1, start_time: @employee.birth + day)
          end
        end
      end
